      ...
      >
        ...
        <div style="padding: 10px">
          <input
            ...
            v-model="this.createChannelModal.name"
            placeholder="Channel Name"
            style="width: 100% !important; padding: 7px 10px"
          />
          <p
            style="color: red"
            v-if="this.createChannelModal.createError != null"
          >
          ...
          <button
            class="button mt-1"
            style="width: 100%"
            @click="createChannel(this.currentServer.id)"
          >
            Create Channel
          </button>
        </div>
  ...
  <div class="channel-bar" v-if="!loading">
    <div v-if="currentServer == 'home'">
      ...
      <template v-if="this.agents.length == 0">
        ...
        >
          <div
            style="text-align: center; width: 200px; color: var(--secondary)"
          >
        ...
        >
          <div style="display: flex; flex-direction: row">
            ...
            <div style="margin-left: 15px">
              ...
              <p style="margin: 0; font-size: 12px" v-if="agent.status == 0">
                Initializing...
              </p>
              <p
              ...
              <p style="margin: 0; font-size: 12px" v-if="agent.status == 2">
                Typing...
              </p>
              <p style="margin: 0; font-size: 12px" v-if="agent.status == 3">
                Sleeping
              </p>
            </div>
          </div>
            ...
            >
              <li>
                <a
                  class="dropdown-item"
                  v-if="agent.status == 3"
                  @click="$emit('open-agent-modal', agent)"
              ...
              <li>
                <a
                  class="dropdown-item"
                  v-if="agent.status == 3"
                  @click="$emit('wake-agent', agent.id)"
                  >Wake agent</a
                >
              </li>
              <li>
                <a
                  ...
                  v-if="agent.status != 3"
                  @click="$emit('sleep-agent', agent.id)"
    ...
    <div v-if="currentServer == 'search'">
      <div
        :class="{ 'channel-thumb': true, active: this.currentSearchTab == 0 }"
        @click="selectSearchTab(0)"
      >
        <span style="font-weight: 700">Search</span>
      </div>

      <div
        :class="{ 'channel-thumb': true, active: this.currentSearchTab == 1 }"
        @click="selectSearchTab(1)"
      >
      ...
      <template v-for="channel in chatChannels">
        ...
        >
          <span style="font-weight: 700"
            ><i class="bi bi-hash"></i> {{ channel.name }}</span
          >
          <br />
        </div>
      </template>
