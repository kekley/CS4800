...
<template>
  ...
  <div class="modal fade" id="createJoinModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" style="max-width: 650px !important;">
      <div class="modal-content"
        style="background: var(--bg-0); padding: 10px; color: white; font-family: 'Ubuntu'; max-width: 650px !important;">

        <div class="row mb-2">
          <div class="col-6" style="margin: 0; padding: 0;">
            <div :class="{ 'slider-tab': true, 'active': (this.createJoinModal.tab == 0) }"
              @click="this.createJoinModal.tab = 0">
              Create a Server
            </div>
          </div>
          <div class="col-6" style="margin: 0; padding: 0;">
            <div :class="{ 'slider-tab': true, 'active': (this.createJoinModal.tab == 1) }"
        ...
        <div v-if="this.createJoinModal.tab == 0" style="padding: 10px;">
          ...
          <p style="color: red;" v-if="this.createJoinModal.createError != null">{{ this.createJoinModal.createError }}
          </p>
  ...
  <div class="loading" v-if="!this.ready || this.needsUsername">
    ...
    <div v-if="this.needsUsername" style="text-align: center; width: 400px;">
      ...
      <p style="margin: 0; margin-top: 15px; font-size: 18px; ">We're excited you're here. We need a name to call you in
      ...
      <p style="margin: 0; color: red; margin-top: 15px;" v-if="this.hasUsernameError">Sorry, that username is already
        taken.</p>
    ...
    <div class="app-window">
      <div style="display: flex; flex-direction: column;">
        ...
        <div class="connection-status">
          <p class="text-success" style="margin: 0; font-size: 18px; font-weight: 700; margin-bottom: 2px;"
            v-if="false">Connected to Web RTC</p>
          <div class="overall-status">
            <div style="display: flex; align-items: center;">
              <div
                :class="{ 'spinner-grow': true, 'text-warning': (this.ws_info.connectionState == null), 'text-success': (this.ws_info.connectionState == 2) }"
            ...
            <p style="font-size: 12px; color: #d3d3d3; margin: 0;">{{ (this.ws_info.latency == null) ? '' :
              `${this.ws_info.latency}ms` }}</p>
          </div>
        </div>
        ...
        <div class="status-center">
          ...
          <div class="dropdown">
            ...
            <div class="dropdown-menu p-3"
              style="width: 300px; background: #151626; border: 2px solid rgba(255, 255, 255, 0.12);">
              <p style="font-size: 20px; color: #f2f2f2; margin: 0; font-weight: 700;">{{ user.given_name }} {{
                user.family_name
                }}</p>
              <p style="margin: 0; color: #f2f2f2; font-size: 15px;" class="mb-3">Signed in as <strong>{{ user.email
                  }}</strong>
              </p>
          ...
          <div style="display: flex; flex-direction: row;">
            <p style="margin: 0; font-size: 20px; margin-left: 20px; cursor: pointer;"><i class="bi bi-mic-fill"></i>
            </p>
            <p style="margin: 0; font-size: 20px; margin-left: 20px; cursor: pointer;"><i
                class="bi bi-volume-up-fill"></i></p>
          </div>
      ...
      <div class="home-content" v-if="this.currentServer == 'home'">
        <div style="text-align: center;">
          ...
          <p style="margin: 0; font-size: 32px; font-weight: 0; margin-top: 25px;"><strong>Welcome, @{{
            userInfo.username
              }}!</strong></p>
          <p style="margin: 0; font-size: 28px; font-weight: 0;">Select a server to get started ({{ this.currentServer
            }})</p>

          <div
            style="display: flex; flex-direction: row; align-items: center; justify-content: center; margin-top: 25px;">
          ...
          <div style="margin-top: 25px;">
            <button class="button" style="width: 300px; background: #444552;" data-bs-toggle="modal"
              data-bs-target="#createJoinModal" @click="this.createJoinModal.tab = 0">Create a Server</button>
              <button class="button" style="width: 300px; background: #444552; margin-left: 15px;"
                data-bs-toggle="modal" data-bs-target="#createJoinModal" @click="this.createJoinModal.tab = 1">Join a
                Server</button>
          </div>
          <CreateServer v-if="isServerCreating" @serverCreated="isServerCreating = false; loadUserServers();"
